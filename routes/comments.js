...
router.get("/:id",async function(req,res) {
    try{
        ...
        res.status(404).json({message:"Oh!! We cannot find this comment, please try again "});
    }
});
...
router.post("/", async(req, res) => { // add comment
    let commentInfo = req.body;
    if (!commentInfo) {
        res.status(400).json({ error: 'Please input a content you want to express' });
        return;
      }
