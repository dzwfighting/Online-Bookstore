...
async function createPost(topic, userId, content, photoArr, tagArr) {
    //when a post is created, the user's postID needs to add a piece of data
...
async function removePost(postId) {
    ...
    await removeAllComments(postId);//delete the data in the comment collection
...
async function addCommentId(postId, commentId) {
    if (!postId || typeof postId !== "string") throw 'You must provide a post id';
...
async function removeCommentId(postId, commentId) {
    if (!postId || typeof postId !== "string") throw 'You must provide a post id';
    if (!commentId || typeof commentId !== "string") throw 'You must provide a comment id';
    let postObjId = ObjectId.createFromHexString(postId);
    let postCollection = await posts();
...
async function removeAllComments(postId) {
    //can delete all the comments which has this postId
